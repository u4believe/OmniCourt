...
def test_web_render_text_mode_works(direct_vm, direct_deploy):
    ...
    Used by OmniCourt's evidence fetching. Validates that mock_web intercepts
    WebRender calls.
    """
    direct_vm.mock_web(
        r".*evidence.*", {"status": 200, "body": "Tracking: never scanned"}
    )
    direct_vm.mock_llm(
        r".*",
        '{"recommended_action": "escalate", "allocation_bps": 0, "reasoning": "x"}',
    )

    contract = direct_deploy("contracts/omnicourt.py")
    contract.open_dispute("person_person", "c", "r", "Never arrived.", "Refund")
    contract.submit_evidence(0, "complainant", "https://evidence.example.com/a")
